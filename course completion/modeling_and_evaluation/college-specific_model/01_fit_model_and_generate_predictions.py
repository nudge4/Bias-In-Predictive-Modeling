# -*- coding: utf-8 -*-
"""
Created on Fri Jun 17 16:45:49 2022
...
def create_confusion_matrix(y_test_pred, threshold, fname):
    cm_arr = confusion_matrix(np.array(test_df.grade), np.where(np.array(y_test_pred) > threshold, 1, 0))
    ...
    y_test_pred_bin = np.where(np.array(y_test_pred) > threshold, 1, 0)
    cm_dict = {}
...
predictors_all = [p for p in predictors if p not in set(l4+l5)]
print(len(predictors_all))
train_df_all = df[df.strm < 2192]
test_df_all = df[df.strm >= 2192]

cname = {'Northern Virginia': "NVCC", "Tidewater": "TCC", "J. Sargeant Reynolds": "JSRCC", "Thomas Nelson": "TNCC", "John Tyler": "JTCC"}

for pr in ['Tidewater', 'J. Sargeant Reynolds', 'Thomas Nelson', 'John Tyler']:
    pr2 = cname[pr]
    print(pr)
    train_df = train_df_all[train_df_all.college == pr]
    test_df = test_df_all[test_df_all.college == pr]
    predictors = [p for p in predictors_all if np.var(train_df[p]) > 0]
    if pr2 == "TNCC":
        predictors = [p for p in predictors if p not in ['cip_12', 'cip_48']]
    original_test_grade = np.array(test_df.grade)
    original_train_grade = np.array(train_df.grade)
    print(train_df.shape,test_df.shape)
    # Fit the RF model
    ...
    pickle.dump(y_test_pred_rf, open("y_test_pred_rf_{}_subgp.p".format(pr2), "wb"))
    pickle.dump(list(test_df.grade), open("y_test_{}.p".format(pr2), "wb"))
    pickle.dump(list(test_df.num_of_prior_terms), open("num_{}.p".format(pr2), "wb"))
    pickle.dump(y_train_pred_rf, open("y_train_pred_rf_{}_subgp.p".format(pr2), "wb"))
    pickle.dump(list(train_df.grade), open("y_train_{}.p".format(pr2), "wb"))
    print("RF for {} subgroup:".format(pr2))
    print("Validation AUC = {}".format(round(roc_auc_score(np.array(test_df.grade), y_test_pred_rf),4)))
    print("Training AUC = {}".format(round(roc_auc_score(np.array(train_df.grade), y_train_pred_rf),4)))
    ...
    print("Best threshold for {0} subgroup = {1}".format(pr2, best_threshold))
    print("Best threshold for {0} subgroup = {1}".format(pr2, best_threshold))
    bas = balanced_accuracy_score(np.array(test_df.grade), np.where(np.array(y_test_pred_rf) > best_threshold, 1, 0))
    print("Balanced accuracy score for {0} subgroup = {1}".format(pr2, bas))
    # pr_rf = create_confusion_matrix(y_test_pred_rf, best_threshold, "RF_1_cm_{}".format(pr2))
    # print(pr_rf)
    print("Pred success rate for {0} subgroup = {1}".format(pr2, np.mean(np.array(y_test_pred_rf) > best_threshold)))
    print("\n\n\n")
    ...
    for r in ['overall', 'white', 'afam', 'hisp', 'asian', 'other']:
        ...
        plt.savefig(r +"_1_{}.png".format(pr2))
    ...
    .round(4).to_csv("success_rate_1_{}.csv".format(pr2), index=False)
    
    success_rate_3 = []
    for r in ['overall', 'white', 'afam', 'hisp', 'asian', 'other']:
        if r == 'overall':
            y_arr = np.array(y_train_pred_rf)
            y_actual = np.array(train_df.grade)
        else:
            y_arr = np.array(y_train_pred_rf)[np.where(np.array(train_df[r]) == 1)[0]]
            y_actual = np.array(train_df.grade)[np.where(np.array(train_df[r]) == 1)[0]]
    ...
    .merge(pd.DataFrame(psr_pval, columns=['subgroup', 'p-value_for_pred_success_rate']), on=['subgroup'], how='left')\
    .loc[:,['subgroup', 'N', 'actual_success_rate', 'p-value_for_actual_success_rate', 'pred_success_rate', 'p-value_for_pred_success_rate']]\
    .round(4).to_csv("success_rate_3_{}.csv".format(pr2), index=False)
    ...
    results_auc_df.to_csv("bias_cstat_{}.csv".format(pr2), index=False)
    pred_score_by_race = pd.DataFrame({'race_column': race_column, 'pred_y': pred_y, 'test_y': test_y})
    pred_score_by_race.to_csv("pred_score_by_race_{}.csv".format(pr2), index=False)
    ...
    results_tpr_df.to_csv("bias_tpr_{}.csv".format(pr2), index=False)
    ...
    results_fpr_df.to_csv("bias_fpr_{}.csv".format(pr2), index=False)
    ...
    results_precision0_df.to_csv("bias_precision0_{}.csv".format(pr2), index=False)
    ...
    results_bas_df.to_csv("bias_bas_{}.csv".format(pr2), index=False)
    ...
    results_pred_score_df.to_csv("bias_pred_score_{}.csv".format(pr2), index=False)
    ...
    results_pred_score_train_df.to_csv("bias_pred_score_train_{}.csv".format(pr2), index=False)
    ...
    for r in ['afam', 'hisp', 'asian', 'other']:
        try:
            ...
            plt.savefig(r +"_predictive_parity_{}.png".format(pr2))
        except:
            pass
    ...
    try:
        for p in [15,55,85]:
            ...
            sub.to_csv("predictive_parity_pct_{0}_{1}.csv".format(p, pr2))
            print("/n/n/n")
    except:
        pass
