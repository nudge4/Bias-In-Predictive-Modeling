...
for pr in ['ENG_111', 'SDV_100', 'ENG_112', 'ITE_115', 'BIO_101']:
    print(pr)
    train_df = train_df_all[train_df_all.course == pr]
    test_df = test_df_all[test_df_all.course == pr]
    ...
    pickle.dump(y_test_pred_rf, open("y_test_pred_rf_{}_subgp.p".format(pr), "wb"))
    pickle.dump(list(test_df.grade), open("y_test_{}.p".format(pr), "wb"))
    pickle.dump(list(test_df.num_of_prior_terms), open("num_{}.p".format(pr), "wb"))
    pickle.dump(y_train_pred_rf, open("y_train_pred_rf_{}_subgp.p".format(pr), "wb"))
    pickle.dump(list(train_df.grade), open("y_train_{}.p".format(pr), "wb"))
    print("RF for {} subgroup:".format(pr))
    ...
    print("Best threshold for {0} subgroup = {1}".format(pr, best_threshold))
    print("Best threshold for {0} subgroup = {1}".format(pr, best_threshold))
    bas = balanced_accuracy_score(np.array(test_df.grade), np.where(np.array(y_test_pred_rf) > best_threshold, 1, 0))
    print("Balanced accuracy score for {0} subgroup = {1}".format(pr, bas))
    # pr_rf = create_confusion_matrix(y_test_pred_rf, best_threshold, "RF_1_cm_{}".format(pr))
    # print(pr_rf)
    print("Pred success rate for {0} subgroup = {1}".format(pr, np.mean(np.array(y_test_pred_rf) > best_threshold)))
    print("\n\n\n")
    ...
    for r in ['overall', 'white', 'afam', 'hisp', 'asian', 'other']:
        ...
        plt.savefig(r +"_1_{}.png".format(pr))
    ...
    .round(4).to_csv("success_rate_1_{}.csv".format(pr), index=False)
    ...
    .merge(pd.DataFrame(psr_pval, columns=['subgroup', 'p-value_for_pred_success_rate']), on=['subgroup'], how='left')\
    .loc[:,['subgroup', 'N', 'actual_success_rate', 'p-value_for_actual_success_rate', 'pred_success_rate', 'p-value_for_pred_success_rate']]\
    .round(4).to_csv("success_rate_3_{}.csv".format(pr), index=False)
    ...
    results_auc_df.to_csv("bias_cstat_{}.csv".format(pr), index=False)
    pred_score_by_race = pd.DataFrame({'race_column': race_column, 'pred_y': pred_y, 'test_y': test_y})
    pred_score_by_race.to_csv("pred_score_by_race_{}.csv".format(pr), index=False)
    ...
    results_tpr_df.to_csv("bias_tpr_{}.csv".format(pr), index=False)
    ...
    results_fpr_df.to_csv("bias_fpr_{}.csv".format(pr), index=False)
    ...
    results_precision0_df.to_csv("bias_precision0_{}.csv".format(pr), index=False)
    ...
    results_bas_df.to_csv("bias_bas_{}.csv".format(pr), index=False)
    ...
    results_pred_score_df.to_csv("bias_pred_score_{}.csv".format(pr), index=False)
    ...
    results_pred_score_train_df.to_csv("bias_pred_score_train_{}.csv".format(pr), index=False)
    ...
    for r in ['afam', 'hisp', 'asian', 'other']:
        try:
            ...
            plt.savefig(r +"_predictive_parity_{}.png".format(pr))
        except:
            pass
    ...
    try:
        for p in [15,55,85]:
            ...
            sub.to_csv("predictive_parity_pct_{0}_{1}.csv".format(p, pr))
            print("/n/n/n")
    except:
        pass
