...
for r in ['white', 'afam', 'hisp', 'asian', 'other']:
    ...
    pickle.dump(y_test_pred_rf, open("y_test_pred_rf_{}_subgp.p".format(r), "wb"))
    pickle.dump(list(test_df.grade), open("y_test_{}.p".format(r), "wb"))
    pickle.dump(list(test_df.num_of_prior_terms), open("num_{}.p".format(r), "wb"))
    pickle.dump(y_train_pred_rf, open("y_train_pred_rf_{}_subgp.p".format(r), "wb"))
    pickle.dump(list(train_df.grade), open("y_train_{}.p".format(r), "wb"))
    print("RF for {} subgroup:".format(r))
    ...
    best_threshold_dict[r] = best_threshold
    print("Best threshold for {0} subgroup = {1}".format(r, best_threshold_dict[r]))
    print("Best threshold for {0} subgroup = {1}".format(r, best_threshold_dict[r]))
    bas = balanced_accuracy_score(np.array(test_df.grade), np.where(np.array(y_test_pred_rf) > best_threshold_dict[r], 1, 0))
    print("Balanced accuracy score for {0} subgroup = {1}".format(r, bas))
    # pr_rf = create_confusion_matrix(y_test_pred_rf, best_threshold_dict[r], "RF_1_cm_{}".format(r))
    # print(pr_rf)
    print("Pred success rate for {0} subgroup = {1}".format(r, np.mean(np.array(y_test_pred_rf) > best_threshold_dict[r])))
    print("\n\n\n")
...
num_y = []
success_rate_1 = []
for r in ['white', 'afam', 'hisp', 'other', 'asian']:
    l1 = pickle.load(open("y_test_pred_rf_{}_subgp.p".format(r), "rb"))
    l2 = pickle.load(open("y_test_{}.p".format(r), "rb"))
    success_rate_1.append((r, len(l2), np.mean(l2), np.mean(np.array(l1) > best_threshold_dict[r])))
    race_column.extend([r]*len(l1))
    pred_y.extend(l1)
    test_y.extend(l2)
    num_y.extend(pickle.load(open("num_{}.p".format(r), "rb")))
...
pred_score_by_race = pd.DataFrame({'race_column': race_column, 'num_y': num_y, 'pred_y': pred_y, 'test_y': test_y})
pred_score_by_race.to_csv("pred_score_by_race_num.csv", index=False)
...
def ba_0(subgp):
    ...
    l2_sub = l2[indices]
    l3_sub = l3[indices]
    N1 = sum(l2_sub <= best_threshold_dict[subgp])
    N2 = sum(l3_sub == 0)
    a1 = 1-np.mean(l3_sub[l2_sub <= best_threshold_dict[subgp]]) #precision_0
    a2 = np.mean(l2_sub[l3_sub == 0] > best_threshold_dict[subgp]) #false positive rate
...
def ba_1(subgp):
    ...
    l3_sub = l3[indices]
    N = sum(l3_sub == 0)
    a = np.mean(l2_sub[l3_sub == 1] > best_threshold_dict[subgp]) #true positive rate
    return subgp, N, a
...
def ba_bas(subgp):
    ...
    l3_sub = l3[indices]
    N = len(indices)
    a = balanced_accuracy_score(l3_sub, np.where(l2_sub > best_threshold_dict[subgp], 1, 0))
    return subgp, N, a
...
test_y_train = []
for r in ['white', 'afam', 'hisp', 'other', 'asian']:
    l1 = pickle.load(open("y_train_pred_rf_{}_subgp.p".format(r), "rb"))
    l2 = pickle.load(open("y_train_{}.p".format(r), "rb"))
    race_column_train.extend([r]*len(l1))
    pred_y_train.extend(l1)
    test_y_train.extend(l2)
...
pred_score_by_race_training = pd.DataFrame({'race_column': race_column_train, 'pred_y': pred_y_train, 'test_y': test_y_train})
pred_score_by_race_training.to_csv("pred_score_by_race_training.csv", index=False)
