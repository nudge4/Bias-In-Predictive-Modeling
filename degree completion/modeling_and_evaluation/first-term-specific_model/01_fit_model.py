...
print("First-term-specific models:")
...
to_drop = [p for p in list(df_new.columns)[10:] if re.search("_fa[2-6]$",p) or re.search("_sp[2-6]$",p) or re.search("_su[2-6]$",p) or re.search("_yr[2-6]$",p)]
predictors = [p for p in list(df_new.columns)[10:] if p not in set(to_drop)]
for p in ['degree_seeking', 'term_cred_att', 'term_gpa', 'prop_comp', 
'withdrawn_prop_comp', 'lvl2_prop_comp', 'dev_prop_comp', 'repeat', "enrolled_nsc", "enrl_intensity_nsc"]:
    df_new.loc[:,p+"_tm1"] = 0
    df_new.loc[:,p+"_tm1"] = df_new.apply(lambda x: x[p+"_fa1"] if x.enrolled_fa1 == 1 else x[p+"_tm1"], axis=1)
    df_new.loc[:,p+"_tm1"] = df_new.apply(lambda x: x[p+"_sp1"] if x.enrolled_sp1 == 1 else x[p+"_tm1"], axis=1)
    df_new.loc[:,p+"_tm1"] = df_new.apply(lambda x: x[p+"_su1"] if x.enrolled_su1 == 1 else x[p+"_tm1"], axis=1)
to_drop = [p for p in list(df_new.columns)[10:] if re.search("_fa1$",p) or re.search("_sp1$",p) or re.search("_su1$",p) or re.search("_yr1$",p)]
predictors = [p for p in predictors if p not in set(to_drop)]
df_new.loc[:,'enrolled_tm1'] = 1
...
to_drop = ['enrolled_pre', 'enrolled_nsc'] + ['enrolled_' + t + str(i) for t in ['tm'] for i in range(1,2)] + ['enrolled_nsc_' + t + str(i) for t in ['tm'] for i in range(1,2)]
predictors = [p for p in predictors if p not in set(to_drop)]
predictors += [e + "_tm1" for e in ['degree_seeking', 'term_cred_att', 'term_gpa', 'prop_comp', 'withdrawn_prop_comp', 'lvl2_prop_comp', 'dev_prop_comp', 'repeat', 'enrl_intensity_nsc']]
